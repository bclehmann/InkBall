...
namespace Where1::InkBall {
	...
	unsigned int SerializedLevel::required_size(PlayableLevel &level_to_serialize) {
		return sizeof(LevelHeader) + level_to_serialize.blocks.size() * sizeof(BlockInformation) + level_to_serialize.pockets.size() * sizeof(PocketInformation) +
	...
	void SerializedLevel::serialize() {
		...
		uint8_t *head = buffer + sizeof(header);
		...
		for(int i = 0; i < header.num_balls; i++){
			...
			ball_head->color = level_to_serialize->pockets[i].color;
